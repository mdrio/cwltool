...
class DockerCommandLineJob(ContainerCommandLineJob):
    ...
    def __init__(
        ...
        super(DockerCommandLineJob,
              self).__init__(builder, joborder, make_path_mapper, requirements,
                             hints, name)
    ...
    def add_file_or_directory_volume(self, runtime: List[str],
        ...
        if not volume.resolved.startswith("_:"):
            _check_docker_machine_path(
                docker_windows_path_adjust(volume.resolved))
    ...
    def create_runtime(
        ...
        #  self.builder.outdir = self.outdir
        self.append_volume(runtime,
        ...
        if not user_space_docker_cmd:
            ...
            if self.networkaccess:
                if runtimeContext.custom_net:
                    runtime.append("--net={0}".format(
                        runtimeContext.custom_net))
            else:
